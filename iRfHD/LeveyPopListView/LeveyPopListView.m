...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions
{
    ...
    if (height < _options.count * 40) {
        height = _options.count * 40;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.imageView.frame = CGRectOffset(cell.imageView.frame, 6, 0);
    cell.textLabel.frame = CGRectOffset(cell.textLabel.frame, 6, 0);
    cell.textLabel.font = [UIFont fontWithName:@"Helvetica" size:15.];
    
    int row = [indexPath row];
    
    id obj = [_options objectAtIndex:row];
